...
The phased-averaged (spacing-averaged) the equations are used. The theory will be documented later.  

====================================
Subgrid configuration 
====================================

To set up a higher-resolution subgrid system, a user should provide a file with data at the subgrid level. To avoid a large space in data/computer memory, only grid points with available subgrid data is provided. For example, you have a computational grid of 200 x 100 with grid size of 2m x 2m. A higher resolution data with 0.5m x 0.5m are available at grid points (150,50) and (150,51). The subgrid ratio is 4 (2/0.5). The subgrid file should contain the grid point numbers followed by 16 (4x4) data with the higher resolution. In this example, two lines (for the two grid points) of data should be provided as below 
...
The figure below shows comparisons of wave surface and wave setup between models with and without block structures.
